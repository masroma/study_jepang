...
@section('page-title', 'Tambah Kisah Sukses')

@section('content')
<div class="space-y-6">
    <!-- Header -->
    <div>
        <h2 class="text-2xl font-bold text-gray-900">Tambah Kisah Sukses</h2>
        <p class="text-sm text-gray-600 mt-1">Tambahkan testimoni dan kisah sukses alumni baru</p>
    </div>

    <!-- Form -->
    <form method="POST" action="{{ url('admin/v2/kisah-sukses/tambah_proses') }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" id="tahun" name="tahun" value="{{ old('tahun') }}" min="2000" max="{{ date('Y') }}"
                            class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-brand-pink focus:ring-2 focus:ring-brand-pink/20 outline-none transition"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="testimoni" name="testimoni" rows="5" required
                            class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-brand-pink focus:ring-2 focus:ring-brand-pink/20 outline-none transition"
                            placeholder="Masukkan testimoni dari alumni">{{ old('testimoni') }}</textarea>
                        @error('testimoni')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
